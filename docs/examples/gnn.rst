...
generate the Code Property Graphs. 

Joern is a tool written in Scala, which means it needs a Java environment to be executed properly. 
This pipeline was tested with the OpenJDK version 17, so if you want to replicate the same 
environment, run the following steps:
...
In this pipeline the normalization step is done after the Code Property Graphs are generated, so the clean-up
...
For the GNN pipeline, the parameters used are:
...
.. code:: bash
    ...
    python ./scripts/run_new_joern.py ${DATASET} cpg \
        --language c

.. code:: bash

    python ./scripts/run_new_joern.py ${DATASET} export \ 
        --repr cpg14 \
        --format dot
...
will be saved as ``complete-cpg.dot``. This step processes all .dot files using the ``networkx`` library, 
eliminating all duplicated edges.
...
6.6. Generating the embeddings using a Transformer model
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
