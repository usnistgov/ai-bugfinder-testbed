...
def process_csv(input_file, test_data_size=0.33, seed=42):
    ...
    x_train, y_train, x_test, y_test = train_test_split(X, y, test_size=test_data_size, random_state=seed)

    LOGGER.info('Training and validation sets created.')
    LOGGER.info('%d instances will be used for training and %d instances for validation' % (x_train.shape[0], x_test.shape[0]))
