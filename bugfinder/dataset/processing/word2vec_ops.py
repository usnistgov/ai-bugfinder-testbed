...
class ReplaceFunctions(DatasetProcessing):
    ...
    def process_file(self, filepath):
        ...
        with open(filepath, 'r') as in_file:
            ...
            for line in code:
                ...
                for function_name in line_functions:
                    if (len({function_name}.difference(main_set)) != 0) and (len({function_name}.difference(keywords)) != 0):
                        if function_name not in function_symbols.keys():
                            function_symbols[function_name] = 'FUN' + str(function_count)
                            function_count += 1

                        ascii_line = re.sub(r'\b(' + function_name + r')\b(?=\s*\()', 
        ...
        LOGGER.debug(
            '%d functions replaced in %s'
            % (function_count, filepath)
        )
    ...
    def process_file(self, filepath):
        ...
        with open(filepath, 'r') as in_file:
            ...
            for line in code:
                ...
                line_vars =  re.findall(r'\b([_A-Za-z]\w*)\b((?!\s*\**\w+))(?!\s*\()', ascii_line)
                ...
                for var_name in line_vars:
                    if (len({var_name[0]}.difference(keywords)) != 0) and (len({var_name[0]}.difference(main_args)) != 0):
                        ...
                        ascii_line = re.sub(r'\b(' + var_name[0] + r')\b(?:(?=\s*\w+\()|(?!\s*\w+))(?!\s*\()',
