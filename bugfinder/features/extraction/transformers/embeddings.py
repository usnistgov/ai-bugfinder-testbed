...
from os.path import join, splitext, split, normpath, sep
...
class TransformerEmbeddings(AbstractProcessing):
    ...
    def execute(self, **kwargs):
        ...
        # TODO: For now, the dataset path being passed needs to include the 'cpgs'
        # folder.
        # Need to think in a better logic to be able to parse the files in different
        # folders.
    ...
    def _process_file(self, dot_folder):
        ...
        LOGGER.debug(
            "%d .dot files found. Looking for the combined file", len(dotfiles)
        )
        ...
        try:
            with open(out_file, "wb") as pkl_file:
                ...
                pickle.dump(cpg_embedding, pkl_file)
    ...
    def _tokenize_nx_graph(self, tokenizer, dotfile):
        ...
        edge_type_map = {"AST": 0, "CFG": 1, "DDG": 2, "CDG": 3}
        ...
        try:
            ...
            for label, code in label_code.items():
                label_node_map[int(label)] = node_id
                node_id += 1
            ...
            for edge in dot_graph.edges(data=True):
                ...
                try:
                    ...
                    edge_vec = self._tokenize_data(
                        edge_type_string, tokenizer, emb_length
                    )
                    ...
                    LOGGER.error(exc)
                    LOGGER.error(
                        "Error while generating the embeddings for the edge features!"
                    )
                ...
                try:
                    ...
                    LOGGER.error(exc)
                    LOGGER.error(
                        "Error while generating the embeddings for the edge type!"
                    )

                edge_types.append([edge_type])

                edges.append(
                    [label_node_map[int(edge[0])], label_node_map[int(edge[1])]]
                )
