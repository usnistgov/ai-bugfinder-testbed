...
class Node2VecModel(DatasetProcessing):
    ...
    def execute(self, name, **kwargs):
        """Run the processing. This function receives the processed dataset, retrieves all edges related to data and control flow (REACHES and FLOWS_TO),
        generates the graphs, run the node2vec algorithm to generate the random walks in the graphs, creates the model and saves it.
    ...
    def _get_all_edges(self):
        ...
        data and control flows (REACHES and FLOWS_TO) as a dataframe, and appends each dataframe in a single one.
...
class Word2VecModel(DatasetProcessing):
    ...
    def get_token_list(self):
        ...
        Returns:
            token_list: list containing all the tokens in the dataset, concatenated to form the corpus
        """
        token_list = list()
...
class Word2VecEmbeddingsBase(DatasetProcessing):
    ...
    def execute(self, **kwargs):
        """Run the processing. Retrieves each tokenized file as a dictionary, loads the model, generates the embeddings for each token in the file,
    ...
    def vectorize(self, model, tokens):
        """Process the token list and generates a matrix containing the token's embeddings.
        The matrix shape is the embedding length X vector_length defined in the initialization of the class.
        If the number of tokens of the instance is lower than the embedding length, the rest of the matrix is populated with zeros.
        If it's greater, the vector is truncated.
    ...
    def get_token_list(self):
        """Reads each file, retrieves the tokens from it and concatenates them in a single list which will be the corpus.
        The difference between this function and the one in the Word2VecModel class is this one saves the tokens as a
        dictionary where the key is the name of the processed file, so it can be identified later for testing/training.

        Returns:
            token_list: list of dictionaries containing all the tokens in the dataset, processed from the files
        """
        token_list = list()
